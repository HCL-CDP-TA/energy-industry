...
export default function EAEnAUHomeScript() {
  useEffect(() => {
    function getInteractURL() {
      return "http://100.89.156.19:9126/interact"
    }

    function readInteractAudience() {
      var resultString = document.cookie.split('; ').filter(cookie => cookie.startsWith('RTP')).map(cookie => {
      ...
      if (resultString === '') {
        resultString = Object.keys(localStorage).filter(key => key.startsWith('RTP')).map(key => key.replace(/^RTP/, '') + "=" + localStorage[key]).join(';')
